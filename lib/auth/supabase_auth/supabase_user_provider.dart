...
class STPMNewsAppDemoSupabaseUser extends BaseAuthUser {
  STPMNewsAppDemoSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> sTPMNewsAppDemoSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = STPMNewsAppDemoSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
